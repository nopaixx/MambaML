...
def pickle_serialize_object_and_uplodad_to_s3(model, project_id, box_id, port_index):
    filename = 'pickle_{}_{}_{}.pickle'.format(str(project_id), box_id, port_index)
    with open(filename, 'wb') as pickle_file:
        pickle.dump(model, pickle_file)
    # TODO uplodad to s3 first
    save_c_file(filename, filename)
    print("Object saved correctly pickle")
    os.remove(filename)
...
def sk_learn_best_estimator_model_serialize_object_and_upload_to_s3(model, project_id, box_id, port_index):
...
class BoxCode():
    ...
    def __init__(self, str_code, box_id, n_inputs,
                 ...
                 project_id, host, serialize_outputs, response=False):
        ...
        if self.serialize_outputs!=None and self.serialize_outputs!='null' and self.serialize_outputs != "[]":
            ...
            for out in s_outputs:
                ...
                if port_index == output_num and (port_type == 'sklearn_model' or port_type == 'sklean_model'):
                    ...
                    return 'sklearnbestestimator_{}_{}_{}.joblib'.format(str(self.project_id), self.box_id, str(port_index))
                elif (port_index == output_num and port_type == 'pickle'):
                    return 'pickle_{}_{}_{}.pickle'.format(str(self.project_id), self.box_id, str(port_index))
        return None
    ...
    def setChangedBox(self, str_code, box_id, n_inputs, 
        ...
        self.__init__(str_code, box_id, n_inputs, n_outputs, json, depend,
                      params, changed, project_id, host, serialize_outputs, self.response)
        return None
                ...
                for out in s_outputs:
                    ...
                    if port_type == 'sklearn_model' or port_type == 'sklean_model':
                       ...
                       sk_learn_best_estimator_model_serialize_object_and_upload_to_s3(self.outputs[port_index], 
                                                                                       ...
                                                                                       self.box_id,
                                                                                       str(port_index))
                    elif port_type == 'pickle':
                        pickle_serialize_object_and_uplodad_to_s3(self.outputs[port_index],
                                                                  self.project_id,
                                                                  self.box_id,
                                                                  str(port_index))
                    else:
...
def run_celery_project(allboxes, project_id, task, host, saver_host, response):
    
    if saver_host != None:
        saver_x = saver_c.Redis(host=saver_host, port=6379, db=0)
    ...
    try:
        ...
        if allboxes is None or task == 'ALL':
            ...
            for x in d_json['nodes']:
                ...
                if (box_type[0:7] == 'Dataset'):
                    ...
                    box = BoxCode("", node_name, 0, 1, d_json,"", "", False, project_id, host, "", response)
                    ...
                    box = BoxCode(python_code, node_name, n_inputs, n_outputs, 
                                  d_json, depend, params, False, project_id, host, serialize_outputs, response)
                    allboxes.append(box)
        ...
        if task == 'ALL':
            ...
            print("TRAY TO TRAIN TASK -->", task, file=sys.stderr)
...
def getOutput(allboxes, box_id, port):
    return "ok"
...
if __name__ == '__main__':
    ...
    while True:
        ...
        try:
            ...
            if task:
                print("NEW TASK", host)
                allproject = run_celery_project(allproject, project_id, task, host, None, False)
            else:
